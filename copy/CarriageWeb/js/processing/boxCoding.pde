...
void draw() {
  ...
  if (mobile) {
    ...
    fill(0, 0, 0);
    text(">>", marginLeftRight/2, height - (fontSizeRef * 3) + marginTopC + 3, promptGap/2, fontSizeRef * 1.5);
  }
...
class Box {
  ...
  void drawPunctuation() {
    ...
    text(openParenthesis, positionH+boxWidth, positionV +3, punctuationGapLR, boxHeight);
    // Only for the closed parenthesis, posHClosedParenthesis custom variable is used, see BoxHandler.pde
    text(closedParenthesis, posHClosedParenthesis, posVClosedParenthesis +3, punctuationGapLR, boxHeight);
    text(openBrackets, positionH - punctuationGapLR, positionV +3 , punctuationGapLR, boxHeight);
    text(closedBrackets, positionH + boxWidth, positionV +3, punctuationGapLR, boxHeight);
    text(comma, positionH - punctuationGapLR, positionV +5, punctuationGapLR, boxHeight);
    fill(0, 0, 0, 255*sin(frameCounter));
    text("▒", posHPrompt + punctuationGapLR, posVPrompt +3, punctuationGapLR, boxHeight);
  }
